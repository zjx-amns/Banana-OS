...
static void mem_pool_init(uint32_t all_memory) {
    ...
    mylog("        Init memory pool done.\n", 1);
}

static void printKernelPoolInfo(struct pool p) {
    mylog("Kernel pool bitmap address: ", 2);
    put_int((int)p.pool_bitmap.bits);
    mylog("; Kernel pool physical address: ", 2);
    put_int(p.phy_addr_start);
    put_char('\n');
}

static void printUserPoolInfo(struct pool p) {
    mylog("User pool bitmap address: ", 2);
    put_int((int)p.pool_bitmap.bits);
    mylog("; User pool physical address: ", 2);
    put_int(p.phy_addr_start);
    put_char('\n');
}
